...
def gather():
    ...
    return {
        ...
        "obsidian": ("🟢 실행중(Sync)" if "Obsidian.app" in sh("ps", "-eo", "command", timeout=8) else "🔴 꺼짐(Sync 중단)"),
...
<div class=kv><div class=k>💎 Obsidian(Sync)</div><div class=v style='font-size:13px'>{esc(d['obsidian'])}</div></div>
</div></div>
