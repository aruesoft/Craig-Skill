...
- **① 수집**: 링크(웹/유튜브/인스타)·텍스트·사진 → `00_Inbox`에 raw 노트(원본+요약+핵심포인트+카테고리 제안+연결 후보).
  ...
  - 인스타/틱톡: **yt-dlp 캡션(description) + 쿠키** + (영상이면 전사).
  - **threads·x·JS/로그인 벽**: `browser_extract`(playwright 헤드리스 렌더 + 쿠키)로 본문 추출. 공개 글은 쿠키 없이도 렌더됨.
    threads는 **스크롤로 작성자 연속 답글(스레드)까지 로드** → Claude가 작성자 원글+본인 답글만 합치고 타인 댓글 무시.
  - 웹: trafilatura → 실패 시 브라우저 렌더. 이미지: Claude 비전. 그래도 내용 없고 텍스트도 없으면 **정크 노트 안 만들고 안내**.
  - 중복(URL 정규화 해시)이면 기존 노트에 재수집 메모만.
- **② 선별·재조합**(`/curate`, 일 22시) — **v2 UX**: 수집 확인 메시지에 `✅ 바로 승격 · 🗑 버림` 버튼(맥락 생생할 때 결정). 밤 스케줄은 **가치 높은 순·최신순 5건만** 카드 발송(수집일·가치근거·**요약 2줄**·병합 제안 포함, 버튼 ✅승인/📁보관/🗑버림/⏰미루기 + **일괄 승인/보관**). 한 번 보낸 카드는 3일 쿨다운(재발송 대신 "응답 대기 n건" 한 줄), 버튼 처리 결과는 **카드 제자리 갱신**(editMessageText), 가치평가는 `_System/curate_pending.json`에 캐시. 승인 시 **기존 주제노트에 병합 우선**(없으면 신규 + 복습 스케줄 시작) + MOC 갱신. 원본은 `status: promoted`. 설정: config `curate.{batch,cooldown_days,snooze_days}`.
...
| (링크/텍스트/사진) | 인박스 수집 |
| `#ai` `#biz` | 카테고리 힌트 |
| `/curate` | 승인 카드 즉시 발송(쿨다운 무시) · 버튼(✅📁🗑⏰)으로 승격/보관/버림/미루기 · 일괄 승인 |
| `/garden` | 링크·MOC 정비 |
| `/review` | 오늘 복습 카드(👍👌👎) |
| `/weekly` | 주간 리트로 |
