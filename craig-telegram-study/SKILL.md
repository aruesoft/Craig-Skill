...
| `/quiz` | 오늘 복습할 카드 세션(버튼 채점, 약점 우선) |
| `/explain 주제` | 파인만 — 내 설명을 Claude가 채점(빈틈 지적) |
| `/weak` | 자주 틀린 약점 카드 |
| `/status` | 대기·전체·오늘 복습·연속일·이해도·인박스 |
| `/plan` | 약점·인박스 기반 개인 학습 계획 |
| `/report` | 주간 복습 리포트(배운 것·연결·약점·다음 주) |

**config**(선택): `quiz_times`(기본 `[8,21]`), `quiz_max`(20), `remind_hour`(9), `report_day`(6=일), `report_hour`(20).
학습 노트에는 SM-2 복습 카드 + 필요 시 `## 기억법(니모닉)` 이 자동 포함됩니다.
