...
`recommended: true`인 코스가 있으면 추천 코스임을 명확히 언급한다.

**등산로 정보 지도 링크는 반드시 포함한다** (코스 정보를 보여줄 때 항상 `🗺️ 등산로 지도` 섹션):
- **공식 등산 지도 우선**: `map_url`이 있으면 `공식 등산 지도: {map_url}`.
- **국립공원인 산**(설악산·지리산·북한산·한라산 등)이면 웹 검색으로 `{산 이름} 국립공원 탐방로 지도`를 찾아
  **국립공원공단(knps.or.kr) 공식 등산지도(PDF/탐방로) 링크**를 넣는다. 예: 설악산 →
  `https://www.knps.or.kr/download/park/map/map120400.pdf`. **검색으로 확인된 실제 URL만 쓰고 지어내지 않는다.**
- **네이버 지도(등산로 표시)**: `https://map.naver.com/p/search/{산 이름}%20등산로`
- **카카오맵(위치·주변, 유지)**: `https://map.kakao.com/?q={산 이름}+등산로`
- 공식 지도를 못 찾으면 네이버·카카오 링크만이라도 반드시 넣는다.
...
- 코스 데이터가 있으면 **모든 코스**를 표로 정리했고, **등산로 지도 링크를 반드시 포함**했다(공식 `map_url`/국립공원공단 지도 우선 + 네이버 지도 등산로 + 카카오맵).
...
- 등산 지도 `map_url`이 없는 산: 국립공원이면 웹 검색으로 국립공원공단 공식 지도를 찾고, 못 찾으면 네이버 지도 등산로·카카오맵 검색 링크로 **항상 지도 링크는 포함**한다(링크는 지어내지 않는다).
