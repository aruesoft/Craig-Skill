...
3. **결과 정리**: 식당명, 대표 메뉴, 특징을 간략히 정리해 하산 지점별로 구분해 보여준다.
   ...
   | {name} | {menu} | {note} |
   ```
4. 검색으로 찾지 못했을 경우 추측해서 식당을 지어내지 말고, 네이버 지도 검색 링크를 안내한다.
...
- 맛집 검색 실패: 추측해서 식당을 지어내지 말고, 네이버 지도 또는 카카오맵에서 "{하산지점} 맛집" 검색을 안내한다.
...
- 하산식 맛집 조회는 `naver-map` 스킬 또는 웹 검색을 활용하며, 하산 지점이 명확하지 않으면 산 근처 대표 마을/읍 기준으로 검색한다.
