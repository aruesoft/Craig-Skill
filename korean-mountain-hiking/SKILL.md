...
**등산로 정보 지도 링크는 반드시 포함한다** (코스 정보를 보여줄 때 항상):
- `map_url`이 있으면 표 아래에 `🗺️ 등산 지도: {map_url}` 로 안내한다.
- `map_url`이 없으면 검색 링크로 대체한다: `🗺️ 등산 지도: https://map.kakao.com/?q={산 이름}+등산로`
  (공백은 `+`로). 링크를 지어내지 말고, 없을 때는 이 검색 링크를 쓴다.
...
- 코스 데이터가 있으면 **모든 코스**를 표로 정리했고, **등산로 지도 링크를 반드시 포함**했다(`map_url` 또는 카카오맵 `{산 이름}+등산로` 검색 링크).
...
- 등산 지도 `map_url`이 없는 산: 링크를 지어내지 말고 카카오맵 검색 링크(`https://map.kakao.com/?q={산 이름}+등산로`)로 대체해 **항상 지도 링크는 포함**한다.
