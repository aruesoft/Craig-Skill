...
def _system_prompt():
    ...
    return (
        ...
        "코스 정보에는 **반드시 🗺️ 등산 지도 링크를 포함**한다: lookup_mountain 결과에 map_url 이 "
        "있으면 '🗺️ 등산 지도: {map_url}', 없으면 '🗺️ 등산 지도: https://map.kakao.com/?q={산이름}+등산로' "
        "형태의 검색 링크를 넣는다({산이름}은 실제 이름, 공백은 +).\n"
