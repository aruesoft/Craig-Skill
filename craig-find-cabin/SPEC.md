...
5. 희망 인원 전부 확보한 타깃은 `done` 처리하고 감시 중단(다른 타깃은 계속). 일부만 확보하면 `party`(총 희망 인원)는 유지한 채 계속 감시.
6. **같은 날 기존 예약 취소 후 합산 재예약(2026-09-03 추가)**: KNPS는 회원당 같은 날 예약 1건만 허용하고 인원은 줄이는 변경만 가능하다("인원수는 기존 예약보다 적은 인원으로만 변경 가능"). 따라서 릴레이 시작 시 마이페이지(`/mypage/dashBoard.do?type=rsvt&prdDvcd=S`, 인원은 `selectReservationDetail.do`)에서 같은 날 활성 예약을 읽어 캡션에 계획을 알리고, 캡차 답장 직후 기존 예약을 취소(`POST /appendix/reservationCancelProc.do` rsvtId+allCancel=Y, 대기는 `POST /mypage/cancelWait.do`)하고 목록 재조회로 검증한 뒤 `기존 인원 + 새 잔여`(희망 인원 상한)로 제출한다. 취소 실패 시 제출하지 않고 새 캡차 재발송, 취소 후 제출 실패(캡차 오답) 시 풀린 자리를 포함해 즉시 재시도.
