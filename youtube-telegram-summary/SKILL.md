...
description: |
  ...
  또한 "텔레그램에서 채널명 보내면 자동 등록", "봇에게 채팅으로 채널 추가/삭제", 상시 리스너(--listen)
  설정·자동 재시작(launchd) 요청도 포함합니다.
  핵심: YouTube RSS(새 영상 감지) → secondb.ai REST API(요약) → Telegram Bot(전송) + 텔레그램 수신 명령(양방향).
---
...
| `monitor.py` | 메인 실행 (감지·요약·전송) + 채널 관리 CLI + **텔레그램 수신 명령 처리(`--listen`)** |
| `claude_summary.py` | secondb 실패 시 Claude 폴백 (유튜브 자막 → Claude API 요약) |
...
python monitor.py --listen              # 텔레그램 명령 상시 대기(즉시 응답)
```

## 텔레그램으로 채널 관리 (양방향 명령)

봇에게 **채팅으로 메시지를 보내면** 채널을 추가/삭제/조회한다. `config.telegram_chat_id` 와
일치하는 채팅만 허용해 타인 조작을 막는다. `getUpdates` 폴링으로 수신하고,
`state.json` 의 `telegram_update_offset` 으로 이미 처리한 메시지 중복을 방지한다.

| 보내는 메시지 | 동작 |
|--------------|------|
| `삼프로TV` / `@3protv` / 채널 URL / `UC...` | 모니터링 채널 **추가** (한글 채널명은 유튜브 검색으로 자동 해석) |
| `/list` | 등록 채널 목록 |
...
**반영 시점 — 2경로 공존**
1. **크론 실행마다 자동 반영** (설정 0): 매 실행 시작 때 `process_incoming_commands()` 가 밀린 명령을 처리 (최대 주기만큼 지연).
2. **즉시 응답(상시 리스너)**: `python monitor.py --listen` long-poll → 수초 내 등록·응답.

공용 코어 `add_channel_core` / `remove_channel_core` / `list_channels_core` 를 CLI(`--add-channel` 등)와
텔레그램 핸들러가 함께 쓴다. 명령 응답은 HTML 파싱 없는 평문(`send_telegram_plain`)으로 안전하게 보낸다.

### 상시 리스너 자동 실행 (즉시 응답, 죽으면 자동 재시작)

macOS launchd 로 `--listen` 을 항상 켜둔다. `KeepAlive`(꺼지면 자동 재시작) + `RunAtLoad`(로그인/재부팅 시 자동 시작):

```bash
python install_schedule.py --install-listener   # 리스너 상시 실행 등록
python install_schedule.py --remove-listener     # 해제(별도 꺼짐 요청 전까지는 계속 살아남)
# 상태: launchctl list | grep youtube-telegram   |   로그: listener.log
```

- Label: `com.craig.youtube-telegram-listener`, plist: `~/Library/LaunchAgents/…plist`
- Windows: `--listen` 를 백그라운드로 직접 실행하거나 작업 스케줄러 '로그온 시' 트리거로 등록 (`--install-listener` 안내 출력 참고)
...
(`claude-haiku-4-5`, 입력 $1/출력 $5 per 1M)** 로 직접 요약해 전송한다 (메시지 제목 "Claude 요약").

- 활성 조건: `config.anthropic_api_key` 또는 env `ANTHROPIC_API_KEY`. 없으면 자동 비활성(secondb만 사용).
- 전제: 자막 있는 영상만. `config.claude_model` 로 모델 변경 가능.
- `_summarize_with_fallback()`: secondb 먼저 → 실패 시 Claude. secondb 세션이 아예 없어도 Claude만으로 동작.

## 참고

- **secondb.ai 요약 quota(사용량 한도)** 가 있다. 한 번에 많은 영상을 몰아 요약하면 429(quota 초과)가
  날 수 있으며, 이 경우 해당 영상은 "본 영상"으로 기록되지 않아 **다음 실행에서 자동 재시도**된다.
  (Claude 폴백을 켜두면 429일 때도 자막 있는 영상은 Claude가 대신 요약한다.)
